---
name: bureau-install
description: Use to hook bureau into a project — adding the gem, mounting the settings engine, supplying the methods it asks the host controller for, and declaring which capabilities the app recognises.
...
- `gem "bureau"` — puts the engine, the settings page and the shipped Profile
  section into the app.
- `mount Bureau::Engine => "/settings"` — serves the settings page and every
...
- `current_person` — the host's answer for who is signed in, and the only one of
  these methods bureau always needs.
...
4. **Add `current_person` to `app/controllers/application_controller.rb`.** It
   ...
   Bureau's controllers inherit from the app's `ApplicationController`, so the
...
5. **Decide which account settings act on.** Define `settings_account` on
   ...
   the account the app calls `current_account`. Bureau asks for
...
**Check it worked.** Start the app, sign in and visit the mount path. Bureau
...
replaces that section, which is bureau-develop's job rather than this one's.

**`can?` and `Bureau.capabilities` become required later.** Installing a gem that
